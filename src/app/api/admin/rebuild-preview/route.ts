...
// POST /api/admin/rebuild-preview
// Body: { slug: string }
// Tự động: download design từ designs bucket → tạo watermark → upload vào previews → update DB
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Download file gốc từ designs bucket
    const { data: fileData, error: dlErr } = await supabaseAdmin.storage
      .from('designs')
      .download(product.file_path)

    if (dlErr || !fileData) {
      return NextResponse.json({ error: 'Không download được file: ' + dlErr?.message }, { status: 500 })
    }
    ...
    return NextResponse.json({ 
      ...
      message: `Preview rebuilt cho ${slug}`
    })
